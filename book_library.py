...
@app.route("/all_books")
def Book_main_page():
    all_books = Books.query.all() #get data from database
    res=[] #create empty array
    for book in all_books: # loop on array
        res.append({"name":book.book_name,"author":book.book_author,"published":book.publish_date,"id":book.book_id}) # add item item from the table to array
    return res # return JSON to clien
...
def add_new_book(): 
    if request.method=='POST':
        ...
        print(book_name, book_author, publish_date, book_type)
